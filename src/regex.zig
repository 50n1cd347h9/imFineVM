...
pub fn match(pattern: []const u8, target: []u8) !?usize {
...
pub fn sub(pattern: []const u8, replacement: []const u8, target: []u8) ![]u8 {
    const slice = try allocator.alignedAlloc(u8, ALIGNOF_REGEX_T, SIZEOF_REGEX_T);
    defer allocator.free(slice);
    ...
    try replace(place, replacement, target);
}

fn replace(place: re.regmatch_t, replacement: []const u8, target: []u8) !void {
