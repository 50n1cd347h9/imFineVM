...
const memBitNot = SubInstructions.memBitNot;
...
pub fn instruction_table() type {
    return struct {
        ...
        pub fn init() *[@intFromEnum(InsCode.count)]*const fn (*Self) void {
            ...
            func_vec[@intFromEnum(InsCode.not)] = not;
            ...
            func_vec[@intFromEnum(InsCode.int)] = int;
            return &func_vec;
        }
    };
}
...
fn arithmetic_switching(self: *Self, execute: fn (Reg, ByteWidth) ByteWidth) ByteWidth {
    ...
    switch (self.ext) {
        ...
        Ext.reg => {
            const src_reg: Reg = getReg(memory[ip + 2]);
        ...
        Ext.reg_ref => {
            const src_ref: Ref = (getReg(memory[ip + 2])).*;
...
pub fn pop(self: *Self) void {
    ...
    switch (self.ext) {
        Ext.reg => {
            const dst_reg: Reg = getReg(self.memory[self.cpu.ip + 2]);
        ...
        Ext.reg_ref => {
            const dst_ref = (getReg(self.memory[self.cpu.ip + 2])).*;
            copy(dst_ref, self.cpu.sp, MACHINE_BYTES);
        },
        else => {
            return;
...
pub fn push(self: *Self) void {
    ...
    switch (self.ext) {
        ...
        Ext.reg => {
            const src = (getReg(self.memory[self.cpu.ip + 2])).*;
        ...
        Ext.reg_ref => {
            const src_ref: Ref = (getReg(self.memory[self.cpu.ip + 2])).*;
            copy(self.cpu.sp - MACHINE_BYTES, src_ref, MACHINE_BYTES);
        },
        else => {
            return;
...
pub fn div(self: *Self) void {
    ...
    self.cpu.gr1 = self.arithmetic_switching(struct {
        fn execute(dst_reg: Reg, src: ByteWidth) ByteWidth {
            dst_reg.* /= src;
            return dst_reg.* % src;
        }
...
pub fn not(self: *Self) void {
    ...
    switch (self.ext) {
        ...
        Ext.reg => {
            const dst_reg = getReg(self.memory[self.cpu.ip + 2]);
            dst_reg.* = ~dst_reg.*;
        },
        Ext.imm_ref => {
            const dst_ref: Ref = fetch(self.cpu.ip + 2, MACHINE_BYTES);
            memBitNot(dst_ref, MACHINE_BYTES);
        },
        Ext.reg_ref => {
            const dst_ref: Ref = (getReg(self.memory[self.cpu.ip + 2])).*;
            memBitNot(dst_ref, MACHINE_BYTES);
        },
        else => {
            return;
...
pub fn _shl(self: *Self) void {
    ...
    _ = self.arithmetic_switching(struct {
        fn execute(dst_reg: Reg, src: ByteWidth) ByteWidth {
            if (getSigned(src) < 0) {
                dst_reg.* = dst_reg.* >> @as(u5, @intCast(-getSigned(src)));
            } else {
